...
def get_wikitext103(nsamples, seed, seqlen, model):
    """
    Wikitext-103 (raw) support.

    - Calibration: sample random documents and slice sequences (avoid concatenating the entire train split).
    - Test: tokenize the full test split into a single contiguous tensor (reasonable size).
    """
    print("get_wikitext103")
    ...
    # Full test split as a contiguous stream (like wikitext2)
    testenc = tokenizer("\n\n".join(testdata["text"]), return_tensors="pt")
    return trainloader, testenc
...
def get_lambada(nsamples, seed, seqlen, model):
    """
    LAMBADA (plain_text) support.

    - Calibration: sample random train examples; slice subsequences when long enough.
    - Eval: tokenize the full validation split into a single contiguous tensor.
    """
    print("get_lambada")
    traindata = load_dataset("lambada", "plain_text", split="train")
    valdata = load_dataset("lambada", "plain_text", split="validation")
    ...
    # Full validation split as a contiguous stream
...
def get_loaders(name, nsamples=128, seed=0, seqlen=2048, model=""):
    ...
    if "wikitext103" in lname or "wikitext-103" in lname:
        return get_wikitext103(nsamples, seed, seqlen, model)

    if "lambada" in lname:
        return get_lambada(nsamples, seed, seqlen, model)
