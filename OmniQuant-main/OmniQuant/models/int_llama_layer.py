...
class QuantLlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # ---- HF compat kwargs (newer transformers) ----
        ...
        **kwargs,
        ...
        """
        HF-compatible QuantLlamaDecoderLayer.

        Key points:
          - accepts newer HF kwargs (past_key_values, cache_position, position_embeddings)
          - ensures hidden_states is a Tensor (unwrap if tuple/list)
          - returns tuple where outputs[0] is Tensor hidden_states
          - if use_cache, returns present_key_value as last element (HF expects that)
        """
        # 0) Unwrap hidden_states if it arrives as (tensor,) or (tensor, ...)
        if isinstance(hidden_states, (tuple, list)):
            if len(hidden_states) > 0 and torch.is_tensor(hidden_states[0]):
                hidden_states = hidden_states[0]
            elif len(hidden_states) == 1 and torch.is_tensor(hidden_states[0]):
                hidden_states = hidden_states[0]

        # 1) HF passes past_key_values (plural) in newer versions; prefer explicit past_key_value if given
        ...
        # 2) Self-attention
        # If HF gave position_embeddings, pass through when possible.
        # Your QuantLlamaAttention doesn't accept it; safest is to ignore unless you add support there.
        ...
        # 4) Build outputs in HF order
        # HF expects: (hidden_states, [attn], [present_key_value])
