...
class QuantLlamaAttention(nn.Module):
    ...
    def __init__(self, org_module: nn.Module, config: LlamaConfig, args=None):
        ...
        # Some transformers versions expose org_module.rotary_emb; others don't.
        # Keep old behavior when present; otherwise build from config.
        # ------------------------------------------------------------------
    ...
    def _init_rotary_emb(self, org_module: nn.Module, config: LlamaConfig):
        # Preferred: preserve the model's exact rotary module if available.
        if hasattr(org_module, "rotary_emb") and org_module.rotary_emb is not None:
            try:
                return copy.deepcopy(org_module.rotary_emb)
            except Exception:
                # If deepcopy fails for any reason, fall back to construction from config.
                pass

        # Fallback 1: newer transformers often accepts config=...
        try:
            return LlamaRotaryEmbedding(config=config)
        except TypeError:
            pass

        # Fallback 2: older signature: (dim, max_position_embeddings, base, device=None)
        ...
        try:
            ...
            # Last resort: try minimal signature variations
            return LlamaRotaryEmbedding(self.head_dim, max_pos, base)

    def _default_position_ids(self, bsz: int, q_len: int, kv_seq_len: int, device: torch.device):
        # If we have past kv, positions should be offset so new tokens start at kv_seq_len - q_len.
    ...
    def _rotary_cos_sin(
        ...
        """
        Support both rotary APIs:
          - old: rotary_emb(x, seq_len=kv_seq_len) -> (cos, sin)
          - new: rotary_emb(x, position_ids) -> (cos, sin)
        """
        # First try the older API used in your original file.
        try:
            return self.rotary_emb(value_states, seq_len=kv_seq_len)
        except TypeError:
            pass

        # Newer API: needs position_ids.
    ...
    def forward(
        ...
        # Compute cos/sin robustly across transformers versions.
        ...
        attn_weights = self.qkt_matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(
            self.head_dim
        )
